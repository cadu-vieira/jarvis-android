...
class JarvisVoiceService : Service() {
    ...
    private var state = VoiceState.WAKE_WORD
        ...
        scope.launch {
            try {
                wakeWordEngine.detections.collect {
                    handler.post { handleWakeWordDetection() }
                }
            } catch (error: Throwable) {
                android.util.Log.e("JARVIS", "Wake word detection stopped unexpectedly", error)
            }
        }
    ...
    private fun handleWakeWordDetection() {
        if (isDestroyed || state != VoiceState.WAKE_WORD) return

        state = VoiceState.ACKNOWLEDGING
        runCatching { wakeWordEngine.stop() }
            .onFailure { android.util.Log.w("JARVIS", "Could not stop wake word engine", it) }

        // Do not open SpeechRecognizer on a timer: wait until the acknowledgement
        // has actually completed. This prevents two clients fighting for the mic.
        speak("Sim, senhor.") {
            if (!isDestroyed && state == VoiceState.ACKNOWLEDGING) {
                startListeningForCommand()
            }
        }
    }

    private fun startWakeWordIfIdle() {
        if (isDestroyed || state != VoiceState.WAKE_WORD) return
        runCatching { wakeWordEngine.start() }
            .onFailure { android.util.Log.e("JARVIS", "Could not start wake word engine", it) }
    }
    ...
    private fun startListeningForCommand() {
        ...
        if (isDestroyed || state != VoiceState.ACKNOWLEDGING) return
        ...
        recognizer = try {
            ...
            android.util.Log.e("JARVIS", "Could not create SpeechRecognizer", error)
            finishCommand()
            return
        }
            ...
            object : RecognitionListener {
                ...
                override fun onResults(results: Bundle?) {
                    ...
                    if (text.isNullOrBlank()) {
                        ...
                        speak(simpleCommand(text)) { finishCommand() }
                    }
                }
            }
        )
        ...
        runCatching { recognizer?.startListening(intent) }
            .onFailure {
                android.util.Log.e("JARVIS", "Could not start SpeechRecognizer", it)
                finishCommand()
            }
    }
    ...
    private fun finishCommand() {
        ...
        handler.postDelayed({ startWakeWordIfIdle() }, 500L)
    }
